...
to autonomously steer itself based on a set of input images. The deep convolutional neural network used performs well with image regression and classification problems. The initial model was inspired by the [NVIDIA Convolutional Neural Network](https://developer.nvidia.com/blog/deep-learning-self-driving-cars/) implemented for their autonomous vehicle.
...
- Training data for steering was done using a keyboard which likely caused abrupt changes in angles. If data was collected on joystick,
...
- For each data point, image from center, left or right was randomly chosen and augmented using random flip
    and random translate to gain more usability from data collected
- Image processing took approximately 27,000 data points and converted it to 9000 data points
- After completing an 80/20 train test split, training data contained 7396 trainable images
...
- I Started with an initial layer of the size of the input shape, then slowly increased number of filters while decreasing kernal size
- The Model used multiple 50% dropout layers for regularization in between the dense layers
...
- The Full neural network yielded 250,000 parameters
...
- Based on the accuracy and loss metrics for the training data, it would be difficult to justify using this model over the base model

<img src="figures/accuracy on trained model.png" alt="alt text" width=400 height=300>

<img src="figures/loss.png" alt="alt text" width=400 height=300>
...
- Even if the model showed reasonable loss and accuracy metrics, I would expect it to drive erratically on an unseen testing track
...
- For better training performance, I plan to collect 5x the amount of data using a joystick instead of a keyboard
- The ultimate ambitious plan is to implement the trained model on a physically marked test track with a raspberry pi attached to an RC Car containing 3 cameras
